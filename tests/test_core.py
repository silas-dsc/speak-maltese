...
def test_new_card_budget_ignores_yesterdays_reviews():
    """`reviews_at >= datetime('now','-1 day')` compared an ISO-8601 timestamp
    ('…T05:34:32+00:00') against SQLite's own format ('… 06:34:32'). Text ordering
    puts 'T' after ' ', so a review 25 hours old still counted as today and the
    learner's daily new-card allowance was spent before they saw a new card."""
    import sqlite3
    from datetime import timedelta
    ...
    for hours, expected in ((25, False), (23, True), (48, False)):
        stamp = db.iso(srs.now() - timedelta(hours=hours))
        counted = bool(conn.execute("SELECT ? >= ?", (stamp, cutoff)).fetchone()[0])
        assert counted is expected, f"{hours}h-old review counted={counted}"


def test_streak_counts_from_the_same_clock_the_reviews_are_stored_in():
    """`date(reviewed_at)` is a UTC date because `iso()` converts on the way in, so
    the walk has to start from the UTC date. Starting from the local one zeroed the
    streak for anyone far enough east — 09:00 in Sydney is the previous day in UTC."""
    from backend import db, srs

    today = srs.now().date().isoformat()
    yesterday = (srs.now() - timedelta(days=1)).date().isoformat()
    assert db._streak([{"d": today}, {"d": yesterday}]) == 2
    assert db._streak([{"d": yesterday}]) == 0, "a gap today ends the streak"


def test_long_phrases_get_distinct_card_ids():
    """Ids key the review history. Plain truncation at 48 characters merged two long
    phrases into one card, silently attaching one phrase's history to the other."""
    stem = "Kollox tajjeb ħafna, grazzi. Il-kont, jekk jogħġbok, u "
    a, b = curriculum._slug(stem + "ilma"), curriculum._slug(stem + "ħobż")
    assert a != b
    assert len(a) <= 48 and len(b) <= 48
    # short ids keep their readable form unchanged, so existing cards keep working
    assert curriculum._slug("Bonġu, kif int?") == curriculum._slug("Bonġu, kif int?")
    assert "-" in curriculum._slug("Bonġu, kif int?")
